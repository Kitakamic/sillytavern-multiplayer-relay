...
**V1 明确不做**：通用附件（M4.5 的卡片/头像资产通道是唯一例外）、语音、房主迁移、P2P 生成、触碰客人自有的聊天与角色数据（插件自建的托管镜像聊天除外，见插件计划 P3）、酒馆服务端插件形态的外壳（推迟到 V2）。
...
**隐私不变量（任何里程碑不得违反，2026-07-11 修订）**：中继永远不接收 API key 与本地文件路径（无任何例外）。角色卡与世界书数据默认同样不接收；唯一例外是房主对单个房间**显式开启分享**的卡片资产（见 M4.5），且中继只做临时中转、到期即删。共享文本仅为重连而临时保存，须有明确的保留期限。
...
### M4.5 — 资产通道（支撑插件 P3 镜像模式，2026-07-11 新增）
...
M0 → M1 → M2 → M3 → M4 → M4.5 → M5，严格串行。M2 完成前不动插件 UI（协议先稳）；M4 放在 M3 之后，保证桥接调试时客人链路已可用作观察端；M4.5 在插件 P3（镜像聊天模式）开工前完成，P3 自身以五项技术验证为开工门槛（见插件计划）。
