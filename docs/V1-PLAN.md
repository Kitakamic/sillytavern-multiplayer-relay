...
### M2.5 — 资产通道（支撑插件 P3 镜像模式；✅ 完成于 2026-07-11，从 M4.5 提前）

> 提前原因：镜像聊天改为 V1 主路线（客人故事主界面），皮肤卡头像与档位二完整卡都走本通道，须在插件 P3 开工前就绪。

- [x] 房间令牌鉴权的 HTTP 端点：`POST /rooms/{roomId}/assets?kind=card|avatar[&ttlSeconds=N]` 上传、`GET /rooms/{roomId}/assets/{assetId}` 下载（与 WS 同端口，走外壳 HTTP 服务）；鉴权用 `auth.hello` 颁发的会话凭据（`x-relay-client-id` + `x-relay-session-token` 头，恒定时间比较 + 房间成员校验），非成员/跨房间一律 403。
...
- [x] 镜像模式的**四项本地技术验证**前置到本阶段开头：消息截获、生成拦截、编辑回滚、卡片程序化导入导出——源码级验证全部可行（SillyTavern 1.16.0，证据与坑位见插件仓库 `docs/MIRROR-FEASIBILITY.md`）。
- [x] `room-store.js`：本地只读投影（成员、时间线、提案队列、副聊天），按 `seq` 应用事件；缺口派发 desync → `room.resume` 兜底。
